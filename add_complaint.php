<?php
// add_complaint.php (Civic Clarity UI)
session_start();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(isset($conn) && !$conn->connect_error) {
        ...
        if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] == UPLOAD_ERR_OK) {
            ...
            // Automatically safely create the uploads directly locally if missing!
            ...
            // Securely move it from PHP temp to long term storage
        ...
        // Insert DB Logic
        $stmt = $conn->prepare("INSERT INTO complaints (user_id, title, category, description, location, attachment) VALUES (?, ?, ?, ?, ?, ?)");
        if ($stmt) {
            $stmt->bind_param("isssss", $user_id, $title, $category, $description, $location, $attachment);
            if ($stmt->execute()) {
...
<div class="mb-10">
<h1 class="text-4xl font-extrabold tracking-tight text-on-surface mb-2">Report a Civic Issue</h1>
<p class="text-on-surface-variant text-lg">Provide details about the infrastructure or service problem you've encountered.</p>
</div>
...
<form action="add_complaint.php" method="POST" enctype="multipart/form-data" class="space-y-8">
<div class="space-y-2">
<label class="text-sm font-bold uppercase tracking-wider text-on-surface-variant" for="title">Complaint Title</label>
<input name="title" class="w-full bg-surface-container-highest border-none rounded-lg px-4 py-3 focus:ring-0 focus:border-b-2 focus:border-primary transition-all outline-none text-on-surface placeholder:text-outline" id="title" placeholder="e.g., Pothole on 5th Avenue" type="text" required/>
</div>
...
<input type="radio" name="category" value="Road" class="hidden" required>
...
<label class="cursor-pointer group flex flex-col items-center justify-center p-4 bg-surface-container-lowest border border-outline-variant/20 rounded-xl hover:bg-primary-fixed transition-all has-[:checked]:bg-primary-fixed has-[:checked]:border-primary/20 has-[:checked]:text-primary">
<input type="radio" name="category" value="Water" class="hidden">
...
<label class="cursor-pointer group flex flex-col items-center justify-center p-4 bg-surface-container-lowest border border-outline-variant/20 rounded-xl hover:bg-primary-fixed transition-all has-[:checked]:bg-primary-fixed has-[:checked]:border-primary/20 has-[:checked]:text-primary">
<input type="radio" name="category" value="Electricity" class="hidden">
...
<label class="cursor-pointer group flex flex-col items-center justify-center p-4 bg-surface-container-lowest border border-outline-variant/20 rounded-xl hover:bg-primary-fixed transition-all has-[:checked]:bg-primary-fixed has-[:checked]:border-primary/20 has-[:checked]:text-primary">
<input type="radio" name="category" value="Garbage" class="hidden">
...
<textarea name="description" class="w-full bg-surface-container-highest border-none rounded-lg px-4 py-3 focus:ring-0 focus:border-b-2 focus:border-primary transition-all outline-none text-on-surface placeholder:text-outline" id="desc" placeholder="Describe the situation in detail..." rows="5" required></textarea>
</div>
